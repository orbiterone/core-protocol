...
contract CompoundLens {
    ...
    function getCompBalanceMetadata(Orbiter orbiter, address account) external view returns (CompBalanceMetadata memory) {
        return CompBalanceMetadata({
            balance: orbiter.balanceOf(account),
            votes: uint256(orbiter.getCurrentVotes(account)),
            delegate: orbiter.delegates(account)
        });
    }
    ...
    function getCompBalanceMetadataExt(Orbiter orbiter, ComptrollerLensInterface comptroller, address account) external returns (CompBalanceMetadataExt memory) {
        uint balance = orbiter.balanceOf(account);
        comptroller.claimComp(account);
        uint newBalance = orbiter.balanceOf(account);
    ...
    function getCompVotes(Orbiter orbiter, address account, uint32[] calldata blockNumbers) external view returns (CompVotes[] memory) {
        CompVotes[] memory res = new CompVotes[](blockNumbers.length);
        for (uint i = 0; i < blockNumbers.length; i++) {
            res[i] = CompVotes({
                blockNumber: uint256(blockNumbers[i]),
                votes: uint256(orbiter.getPriorVotes(account, blockNumbers[i]))
            });
        }
        return res;
    }
